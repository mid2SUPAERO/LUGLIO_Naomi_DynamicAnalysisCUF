# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 08:58:14 2019
...
Con.append([-h/2,h/2,h/2,h/2,-h/2,-h/2,h/2,-h/2])
...
def Func(el,w,yq,zq):
    ...
    lqy=Con[el][2]-Con[el][0]
    lqz=Con[el][1]-Con[el][7]
    alphaq=(yq-Bar[el][0])/(lqy/2)
    betaq=(yq-Bar[el][1])/(lqz/2)
    ...
    for i in range(0,len(yk)):
        for j in range(0,len(zk)):
            for k in range(0,len(xk)):
                el=80
                for e in range(0,ne_t):
                    if Con[e][0]<=yk[i]<=Con[e][2] and Con[e][5]<=zk[j]<=Con[e][1]:
                        el=e 
                if el!=80:
                    uy=0
                    uz=0
...
plot3dmode(findindex(7),5) #it should represent the first egenvector with a scalefactor equal to 5               