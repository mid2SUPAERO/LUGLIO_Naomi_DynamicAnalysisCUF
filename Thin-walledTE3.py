# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 10:54:01 2019
...
def integrand(phi,r,x,c,b,n,a,k,p):
    global F
    F.clear()
...
def integral(c,b,n,a,k,p):
    return (integ.tplquad(integrand,-le/2,le/2,lambda x: 0, lambda x: R,lambda x,r: 0, lambda x,r:2*Ma.pi, args=(c,b,n,a,k,p),epsabs=100, epsrel=100)[0])-(integ.tplquad(integrand,-le/2,le/2,lambda x: 0, lambda x: R-t,lambda x,r: 0, lambda x,r:2*Ma.pi, args=(c,b,n,a,k,p),epsabs=100, epsrel=100)[0])
...
def Mintegrand(phi,r,x,c,b,k,p):
        global F
        global N
...
def Mintegral(c,b,k,p):
    return (integ.tplquad(Mintegrand,-le/2,le/2,lambda x: 0, lambda x: R,lambda x,r: 0, lambda x,r:2*Ma.pi, args=(c,b,k,p),epsabs=1.49, epsrel=1.49)[0])-(integ.tplquad(Mintegrand,-le/2,le/2,lambda x: 0, lambda x: R-t,lambda x,r: 0, lambda x,r:2*Ma.pi, args=(c,b,k,p),epsabs=1.49, epsrel=1.49)[0])
...
def plot3dmode(nm,scale):
    ...
    yk=np.array(list(np.arange(-(R),R+0.05,0.05))) 
